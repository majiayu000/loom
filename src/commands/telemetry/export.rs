...
pub(super) fn export_csv(entries: &[TelemetryLogEntry]) -> String {
    let mut body = String::from(
        "schema_version,event_id,event_type,skill_id,skillset_id,agent,workspace_hash,session_id_hash,timestamp,tokens_in,tokens_out,commands,duration_ms,success,baseline_delta,feedback,safety_findings,dependency_findings,raw_prompt_stored,raw_code_stored,redacted\n",
    );
    for entry in entries {
