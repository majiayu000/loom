...
- [ ] `SP534-T003` Owner: tooling | Dependencies: `SP534-T002` | Done when: guard wired into Makefile and CI verify job | Verify: `make module-ceiling module-ceiling-test`; CI verification required on PR
...
- [ ] `SP534-T005` Owner: verification | Dependencies: all prior | Done when: full checks pass | Verify: `cargo check --workspace --all-targets --all-features && make check`; CI verification required on PR
