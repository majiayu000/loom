...
September 2026: tests/provision_materialize.rs covers tar and shell import, active-view bytes, no-execution behavior, existing-directory refusal, path escape refusal, Devcontainer directory export, and approved Codespaces local apply. tests/provision_apply.rs also covers Remote local setup apply.
...
- [x] `SP382-T004` Owner: export-import | Done when: shell/tar export and import dry-run are deterministic and never include secret values; devcontainer export writes a new directory, and import materializes verified files without executing them | Verify: `cargo test --test provision_export_import`
...
The follow-up directory contract writes reviewed Devcontainer/Codespaces files
into a new output directory. Shell/tar import materializes verified files into
a new directory. Remote apply writes the local setup script only; scripts and
remote connections are never executed by these commands.
...
- Plan id lookup loads the durable reviewed plan and never regenerates file
  previews from current registry state during apply/export/doctor.
...
- Devcontainer export format is split into a follow-up artifact contract instead
  of inventing output semantics in this packet.
