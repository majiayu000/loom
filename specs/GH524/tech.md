...
loom skill converge <skill> ... --apply --plan-digest <digest> --idempotency-key <key>
```

`--dry-run` 与 `--apply` 必须二选一；`--apply` 必须携带前一次 dry-run 返回的
`--plan-digest`。`--push-remote` 只控制最后 transport 阶段。
...
6. dry-run 不持久化 plan；apply 必须接收 caller 提交的 `--plan-digest`，在锁内根据相同输入
   重新计算并与之比对。缺少 digest 或比对不一致时必须在任何写入前阻断；apply 不得绕过
   已审阅的 dry-run effect plan。
