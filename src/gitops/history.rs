...
use super::{
    EMPTY_TREE_SHA, HISTORY_ARCHIVES_DIR, HISTORY_BRANCH, HISTORY_BRANCH_REF,
    HISTORY_COMPACT_AFTER_SEGMENTS, HISTORY_RETAIN_ARCHIVES, HISTORY_RETAIN_RECENT_SEGMENTS,
    HISTORY_SEGMENTS_DIR, HISTORY_SNAPSHOT_FILE, ORIGIN_HISTORY_BRANCH_REF, TempFile,
    ahead_behind_refs, ensure_local_identity, hash_object_bytes, hash_object_file, read_blob,
    remote_exists, remote_tracking_history_exists, repo_is_initialized, run_git,
...
pub fn mirror_history_segment(
    ...
    let segment_name = segment_path
        ...
        .to_string_lossy()
        .to_string();
    let segment_ref_path = format!("{}/{}", HISTORY_SEGMENTS_DIR, segment_name);
...
struct HistoryRetentionSummary {
    compacted_segments: usize,
    rolled_archives: usize,
}
...
#[derive(Debug, Clone)]
struct ComposedHistoryState {
    archives: BTreeMap<String, String>,
    segments: BTreeMap<String, String>,
    snapshot_blob: String,
    retention: HistoryRetentionSummary,
}
...
fn synthesize_history_snapshot_blob(
    ctx: &AppContext,
    archives: &BTreeMap<String, String>,
    segments: &BTreeMap<String, String>,
) -> Result<String> {
...
fn history_tree_entries(state: &ComposedHistoryState) -> Vec<HistoryTreeEntry> {
...
fn build_tree_from_entries(ctx: &AppContext, entries: &[HistoryTreeEntry]) -> Result<String> {
...
fn compact_local_history_branch(
    ctx: &AppContext,
    local: &HistoryBranchState,
    strategy: HistoryRepairStrategy,
) -> Result<HistoryRepairReport> {
