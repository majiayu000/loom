...
`src/contract.rs`），JSON envelope additive 增加 `cli_contract_version`。crate semver 描述发布版本，
...
3. 调用同一 Clap parser，不执行 `App::execute`；解析成功后还要依据 Clap visibility metadata/public
   allowlist 拒绝 hidden/deferred command path；
...
或 fixture 未实际发出对应 emitter id 时 gate 失败。随后从完整 emitter fixture matrix 收集实际 JSON，
...
当前 `Cargo.toml` 只有 binary target，integration test 无法直接 import `Cli`。实现前必须由
maintainer 批准并记录 ADR，二选一：

1. 抽出最小 parser library surface，binary 与 checker 共用；
2. 增加不执行 mutation 的只读 contract checker surface，由测试以进程方式调用。

在该决策前，规格不得声称现有 integration test 可直接调用 `Cli::try_parse_from`。
...
3. 兼容范围写入 append-only `docs/cli-contract-history.toml`；缩小 Skill range 必须新增历史记录并
   同时修改 `CHANGELOG.md`/migration note，由测试检查同一 diff 中存在且旧记录未被覆写。
...
| --- | --- | --- |
| B-001 | envelope + Skill compatibility metadata | `cargo test cli_contract_version_is_exposed_and_declared` |
| B-002 | `loom-registry` compatibility route | `cargo test --test agent_contract_surfaces incompatible_cli_blocks_mutation` |
| B-003 | surface + emitter + Panel mutation mapping coverage | `cargo test --test agent_contract_surfaces inventory_covers_public_surfaces && cargo test --test agent_contract_surfaces emitter_inventory_is_complete && cargo test --test agent_contract_surfaces panel_mutations_are_mapped` |
| B-004 | shared Clap parser + public visibility checker | `cargo test --test agent_contract_surfaces executable_examples_parse && cargo test --test agent_contract_surfaces hidden_commands_fail` |
...
2. **命令 surface 扫描性能**：选择 parser library 时只 build 一次 test binary 并在内存验证；选择
   进程 checker 时复用单一 checker process/batch 输入，避免每个 argv 启动一次进程。
...
- 维护者需要批准 parser 暴露方式；这是 implementation-ready 前的 architecture gate。
