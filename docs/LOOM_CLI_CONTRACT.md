...
1. `plan use` validates the skill exists, records the current registry `HEAD`, records the current skill source digest, lists effects/conflicts/risks, and returns a `plan_id`
...
3. `apply` is safe to retry with the same `--idempotency-key`; a successful prior apply for the same plan/key returns the recorded result with `idempotent_replay=true`
