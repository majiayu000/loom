...
- [x] `SP600-T3` Owner: parser | Dependencies: SP600-T2 | Done when: provenance-aware tokenizer 保留 quote/continuation/conditional 状态，heredoc 与 HOME mutation 后全部 home-derived prefix fail closed，Windows separator 可解析；逐 verb grammar 正确处理 sed program、option values、tail follow、`--` 与复杂 substitution | Verify: `cargo test --locked --lib commands::telemetry::ingest::codex::tool_read::shell` | Covers: B-001, B-002, B-008
...
- [x] `SP600-T5` Owner: Codex ingest | Dependencies: SP600-T2, SP600-T3, SP600-T4 | Done when: typed rejection 接入 `ParseOutcome`，UTF-8 顶层 nested exec 合法而 dormant body 忽略；相同 turn_id context 保留 read dedupe/ordinal，真实新 turn 重置；legacy injection tests 不变 | Verify: `cargo test --locked --lib commands::telemetry::ingest::codex` | Covers: B-004, B-005, B-007
