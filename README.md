...
- 远端同步：`remote set/status` + `sync push/pull/replay`
- 离线暂存：`state/pending_ops.jsonl`
- 纯文件元数据：`state/locks/`、`state/targets.json`、`state/pending_ops.jsonl`
- `doctor`：包含 `git fsck` 和队列/目标文件检查
- 本地 panel：`loom panel --port 43117`

## 命令

```bash
loom init [--from-agent claude|codex|both] [--target claude|codex|both] [--copy] [--force] [--backup-dir <dir>] [--skip-backup]
loom init --wizard
loom add <path|git-url> --name <skill>
loom import --source <dir> [--skill <name>] [--link] [--target claude|codex|both] [--copy] [--force]
loom import --from-agent claude|codex|both [--skill <name>] [--link] [--target claude|codex|both] [--copy] [--force]
loom link <skill> --target claude|codex|both [--copy]
loom save <skill>
loom snapshot <skill>
loom release <skill> <version>
loom rollback <skill> [--to <ref> | --steps <n>]
loom diff <skill> <from> <to>
loom status
loom doctor
loom remote set <git-url>
loom remote status
loom sync push
loom sync pull
loom sync replay
...
loom init --from-agent both --target both

# 交互式终端选择模式（推荐手动使用）
loom init --wizard

# 从现有 agent 目录导入，并立刻重建 symlink
loom import --from-agent both --link --target both
```
