...
#[test]
fn remote_url_returns_none_when_origin_is_unconfigured() {
    let (ctx, dir) = fresh_repo("remote-url-unconfigured");

    let result = remote_url(&ctx).expect("remote_url should succeed for a fresh repo");
    assert!(
        result.is_none(),
        "fresh repo without origin should yield Ok(None), got {result:?}"
    );
...
#[test]
fn remote_url_returns_err_when_root_is_not_a_git_repository() {
    let dir = std::env::temp_dir().join(format!("loom-gitops-not-a-repo-{}", Uuid::new_v4()));
    fs::create_dir_all(&dir).expect("create temp dir");
    let ctx = AppContext::new(Some(dir.clone())).expect("build AppContext");

    let err = remote_url(&ctx)
        .expect_err("remote_url must Err in a non-git directory rather than masking it as None");
    let message = format!("{err}");
    assert!(
        message.contains("not a git repository"),
        "error must carry the underlying git stderr: {message}"
    );
