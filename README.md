...
| `loom plan use` / `loom plan converge` / `loom apply` | Persist a guarded use or convergence plan, then execute supported plans with idempotency | Agents need a retry-safe plan/apply protocol for higher-risk flows; convergence apply remains disabled until its executor lands | Command audit + source/target/registry metadata |
