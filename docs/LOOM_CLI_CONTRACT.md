...
MCP provisioning is plan-first. Plan, requirement, doctor, and catalog commands are read-only. Apply consumes a durable reviewed plan id or explicit plan artifact, requires an idempotency key and approval tokens for risky actions, revalidates source/config preimages, writes Codex config atomically, and never stores secret values.
...
7. `mcp apply` writes only reviewed Codex `mcp_servers` config, stores environment variable names as references, and returns restart guidance when it changes config
