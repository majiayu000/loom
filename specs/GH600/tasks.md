...
- [x] `SP600-T3` Owner: parser | Dependencies: SP600-T2 | Done when: provenance-aware tokenizer 保留 quote/continuation/conditional 状态，heredoc 与 HOME mutation fail closed，Windows separator 可解析；逐 verb grammar 正确处理 sed program、option values、tail follow、`--` 与复杂 substitution | Verify: `cargo test --locked --lib commands::telemetry::ingest::codex::tool_read::shell` | Covers: B-001, B-002, B-008
