...
pub fn remote_url(ctx: &AppContext) -> Result<Option<String>> {
    ...
    // git exits 2 with "No such remote 'origin'" when the remote is simply
    // unconfigured. Treat any other failure (e.g. exit 128 "not a git
    // repository", permission errors) as a real error so callers can
    // surface it rather than conflate it with "no remote configured".
