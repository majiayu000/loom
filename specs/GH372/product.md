...
```bash
loom skill improve <skill> [--agent <agent>] [--workspace <path>] [--baseline <ref>] [--dry-run]
loom skill regression <skill> [--agent <agent>] [--from <ref>] [--to working-tree]
loom skill save <skill> --preflight [--message <msg>]
loom skill release <skill> <version> --preflight
```
...
7. real eval compare if #369 runner is available and explicitly selected;
...
2. runs same gates;
3. blocks tag/release if gates fail;
4. includes full report in error details.
...
4. `skill release --preflight` blocks on the same gates and requires clean source state.
