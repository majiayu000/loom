...
pub async fn run_panel(ctx: AppContext, port: u16) -> Result<()> {
    ...
    let app = Router::new()
        ...
        .route("/api/v1/targets", get(registry_targets))
        .route("/api/v1/bindings", get(registry_bindings))
        .route("/api/v1/projections", get(registry_projections))
        .route("/api/v1/ops", get(registry_ops))
