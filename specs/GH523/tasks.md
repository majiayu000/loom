...
- [ ] `SP523-T001` Owner: CLI contract | Dependencies: merged architecture-decision spec | Done when: 独立 SemVer `CLI_CONTRACT_VERSION` 以字符串 `cli_contract_version` 字段进入 JSON envelope，agent-facing additive/minor、breaking/major、non-capability-fix/patch 规则写入 ADR 与 CLI contract | Verify: `cargo test cli_contract_semver_is_exposed_and_declared && cargo test contract_additive_capability_requires_minor_bump` | Covers: B-001, B-010
