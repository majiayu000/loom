...
use crate::cli::{
    ...
    ProjectionMethod, ProvisionCommand, RolesCommand, SkillActiveCommand, SkillAuthorCommand,
    SkillCommand, SkillCompileCommand, SkillOrphanCommand, SkillTrashCommand, SkillsetCommand,
...
pub(crate) fn command_name(command: &Command) -> &'static str {
    match command {
        ...
        Command::Skill { command } => match command {
            ...
            SkillCommand::Stats(_) => "skill.stats",
            ...
            SkillCommand::Author { command } => match command {
                SkillAuthorCommand::Draft(_) => "skill.author.draft",
                SkillAuthorCommand::Extract(_) => "skill.author.extract",
                SkillAuthorCommand::Rewrite(_) => "skill.author.rewrite",
                SkillAuthorCommand::TuneDescription(_) => "skill.author.tune_description",
                SkillAuthorCommand::GenerateEvals(_) => "skill.author.generate_evals",
                SkillAuthorCommand::ApplyPatch(_) => "skill.author.apply_patch",
                SkillAuthorCommand::New(_) => "skill.author.new",
            },
