...
| Native | 9 项测试通过；包括路径/参数边界、包摘要、链接和 FIFO 拒绝、非 UTF-8 名称拒绝。`desktop-tests.log` |
...
尚未验收：真实托管 OTP/Keychain 全链路、干净机器安装、Windows、各 Agent 的真实新会话加载、备份恢复演练、限流部署及三团队内测。归档后历史安装恢复的专用 UI 入口未实现，归档技能安装按钮禁用；未归档技能可手动选择历史版本建立固定版本计划。源码编译及模拟浏览器数据不替代上述验收。
