...
2. `/api/v3/ops` returns the entire `operations.jsonl` journal with full row payloads.
...
2. Paginate and bound `/api/v3/ops`, returning compact row summaries instead of full operation payload blobs.
